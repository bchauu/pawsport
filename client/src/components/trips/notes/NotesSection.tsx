        ...
        <View>
            ...
            {categories.map((category) => (
              ...
              >
                <Text
                  style={[
                    styles.buttonText,
                    activeCategory === category.value && styles.activeButtonText,
                  ]}
                >
                  {category.label}
                </Text>
          ...
          <ScrollView>
            ...
            {notes
              ...
              .flatMap((listItem) =>
                listItem.notes.filter((note) => note.category === activeCategory) // Filter by active category
              )
              .map((note, index) => (
                <View key={index}>
                  <Text style={theme.personalList.notesItem}>{note.message}</Text>
                  <Text>{note.user}</Text>
                </View>
              ))}
            ...
            {notes
              ...
              <Text style={styles.noNotesText}>No notes available for this category.</Text>
            )}
          </ScrollView>
        </View>
      )}
