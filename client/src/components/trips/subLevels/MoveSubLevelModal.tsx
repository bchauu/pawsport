...
const MoveSubLevelModal = ({ item, allTrip, tripOrder, setTripOrder,setHighestValueSubLevel, highestValueSubLevel, subLevels }) => {
  ...
  const queryChangeSubLevel = async (payload) => {
    ...
    try {
      const response = await axios.post(`${apiUrl}/trips/lists/places/changesublevel`, payload, {
        headers: {
  ...
  const handleMoveToNewSubLevel = () => {
    ...
    const payload = {movedTripItem: {id: item.id, subLevel: selectedSubLevel, value: Number(highestValueSubLevel[selectedSubLevel]) +1}}

    setTripOrder((prev) => (
        {
            ...prev,
            [item.id]: {"subLevel": selectedSubLevel, value: Number(highestValueSubLevel[selectedSubLevel]) +1}
        }
    ))
    ...
    for (const id in tripOrder) {
      ...
      if (tripOrder[id].value > tripOrder[itemId].value && tripOrder[id].subLevel == currentSubLevel) {
  ...
  const handleSelectedSubLevel = async (subLevelName) => {
    ...
    for (const id in tripOrder) {
        if (tripOrder[id].subLevel === subLevelName || tripOrder[id].subLevel === null && subLevelName.toString() == 'null')  {
