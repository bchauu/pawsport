...
const RemoveSubLevel = ({changeItemCategory, subLevel, setSubLevels}) => {
  ...
  const handleRemoveSubLevel = async () => {
    try {
      const response = await axios.delete(
        `${apiUrl}/trips/lists/deleteSubLevel`,
        {
          data: {
            id: subLevel.id,
          },
      ...
      setSubLevels(prevValue =>
        prevValue.filter(name => name.id !== subLevel.id),
      );
  ...
  return (
    <View>
      {/* <ConfirmationModal handleConfirmation={handleRemoveSubLevel} /> */}
    </View>
  );
};
